...
{
	return glm::gtc::angleAxis(ang, v);
	/*Float sAng;
	ang *= 0.5f;
...
{
	return glm::gtc::quat_cast(glm::gtx::eulerAngleYXZ(umath::deg_to_rad(ang.y), umath::deg_to_rad(ang.p), umath::deg_to_rad(ang.r)));
...
bool uquat::cmp(const Quat &a, const Quat &b, float epsilon) { return (glm::abs(a.x - b.x) <= epsilon && glm::abs(a.y - b.y) <= epsilon && glm::abs(a.z - b.z) <= epsilon && glm::abs(a.w - b.w) <= epsilon) ? true : false; }

Vector3 uquat::forward(const Quat &q) { return Vector3(2 * (q.x * q.z + q.w * q.y), 2 * (q.y * q.z - q.w * q.x), 1 - 2 * (q.x * q.x + q.y * q.y)); }
...
Quat uquat::slerp(const Quat &q1, const Quat &q2, Float factor) { return glm::gtc::slerp(q1, q2, factor); }
Quat uquat::lerp(const Quat &q1, const Quat &q2, Float factor) { return glm::gtc::lerp(q1, q2, factor); }
...
{
	q = glm::gtc::rotate(q, ang.r, uvec::RIGHT);
	q = glm::gtc::rotate(q, ang.p, uvec::FORWARD);
	q = glm::gtc::rotate(q, ang.y, uvec::UP);
}
