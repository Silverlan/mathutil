...
double umath::geometry::calc_volume_of_polyhedron(const std::vector<Vector3> &verts, const std::vector<uint16_t> &triangles, Vector3 *centerOfMass)
{
	...
	return calc_volume_of_polyhedron(
	  [&verts, &triangles, i, numIndices](const Vector3 **v0, const Vector3 **v1, const Vector3 **v2) mutable -> bool {
		  if(i >= numIndices)
			  return false;
...
static Vector3 calc_cone_surface_normal(Vector3 coneCenter, const Vector3 &coneDir, float coneHeight, Vector3 pointOnSurface, float radiusAtPoint)
{
	...
	auto v = Vector3 {pointOnSurface.x - coneCenter.x, pointOnSurface.y - coneCenter.y, 0.f};
	auto l = uvec::length(v);
	if(l > 0.f)
		v /= l;

	auto r = Vector3 {(radiusAtPoint != 0.f) ? (v.x * coneHeight / radiusAtPoint) : 0.f, (radiusAtPoint != 0.f) ? (v.y * coneHeight / radiusAtPoint) : 0.f, (coneHeight != 0.f) ? (-radiusAtPoint / coneHeight) : 0.f};
	uvec::rotate(&r, rot);
	l = uvec::length(r);
	if(l > 0.f)
		r /= l;
